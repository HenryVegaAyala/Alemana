...
$NewFecIng = Yii::app()->dateFormatter->format("dd MMMM y", strtotime($FecIng));
$NewFecEnv = Yii::app()->dateFormatter->format("dd MMMM y", strtotime($FecEnv));
...
<table border="0" class="table">
     <tr>
        <td width="50%" >Señor(es): ' . $row['DES_CLIE'] . ' - ' . $row['DES_TIEN'] . '</td>
        <td width="30%" >N° de RUC: ' . $row['NRO_RUC'] . ' </td>
        <td width="20%" >N° O.C: ' . $oc . ' </td>
     </tr>
</table>
...
{
    if ($Estado == 9) {
        return  "ORDEN DE COMPRA "."ANULADO";
    } else
